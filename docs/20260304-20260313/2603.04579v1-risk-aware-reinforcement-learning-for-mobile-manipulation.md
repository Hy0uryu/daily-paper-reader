...
method: 采用分布强化学习训练教师策略，通过失真风险度量调整优势估计，并利用模仿学习蒸馏出基于深度信息的视觉运动策略。
result: 实验证明该策略在未建模环境中能表现出明显的风险感知行为，并有效提升了任务执行的最差情况性能。
conclusion: 本研究首次实现了可调节风险敏感度的移动操作视觉运动策略，为机器人在复杂动态环境中的安全应用提供了新思路。
---

## 摘要
为了让机器人成功从实验室环境过渡到日常环境，它们必须开始推理与其动作相关的风险，并做出明智的、具有风险感知能力的决策。对于执行移动操作（mobile manipulation）任务的机器人来说尤其如此，这类任务涉及在动态、非结构化空间中的交互与导航。然而，现有的移动操作机器人全身控制器通常缺乏在不确定性下进行风险敏感决策的显式机制。据我们所知，我们首次实现了：(i) 学习以第一人称深度观测为条件的移动操作风险感知视觉运动策略，且风险敏感度在运行时可调；(ii) 证明了风险感知行为可以通过模仿学习（IL）转移到以第一人称深度观测为条件的视觉运动策略中。我们的方法首先使用分布强化学习（DRL）训练一个特权教师策略，并配备一个风险中性的分布评论家（distributional critic）。然后，将失真风险度量（distortion risk-metrics）应用于评论家预测的回报分布，以计算风险调整后的优势估计，并用于策略更新，从而实现一系列风险感知行为。随后，我们通过模仿学习蒸馏教师策略，以获得以第一人称深度观测为条件的风险感知学生策略。我们进行了广泛的评估，结果表明，我们训练的视觉运动策略在未建模环境中执行反应式全身运动时，能够利用实时深度观测进行感知，并表现出风险感知行为（特别是实现了更好的最坏情况性能）。
