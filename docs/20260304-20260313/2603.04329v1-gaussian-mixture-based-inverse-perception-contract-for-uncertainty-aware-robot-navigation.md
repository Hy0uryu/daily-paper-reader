...
tags: ["query:hr-rob"]
score: 7.0
evidence: 用于导航的不确定性感知感知
tldr: 针对复杂环境下机器人导航的感知不确定性问题，本文提出了基于高斯混合模型的逆感知契约（GM-IPC）。传统方法使用单一椭圆集合表示误差，往往过于保守且无法处理多峰分布。GM-IPC 通过高斯混合模型生成的椭圆集合并集来表征不确定性，并结合概率包含、分布匹配和空域惩罚的训练框架，实现了更精细、非凸的误差建模。该方法在保证安全性的同时，显著降低了导航的保守性，提升了机器人在复杂环境中的运动性能。
selection_source: fresh_fetch
motivation: 传统的逆感知契约采用单一椭圆集合，难以捕捉多峰且复杂的感知误差结构，导致导航行为过于保守。
method: 提出 GM-IPC 框架，利用高斯混合模型生成的多个椭圆集合并集来表示不确定性，并通过专门的损失函数进行端到端训练。
result: 该方法能够提供具有形式化保证的精细不确定性表征，在下游规划中实现了更具适应性且安全的机器人运动。
conclusion: GM-IPC 有效解决了感知误差的多峰性建模问题，在维持安全性的前提下提升了机器人在复杂环境中的导航效率。
---

## 摘要
在拥挤环境中的可靠导航不仅需要准确的感知输出，还需要配备适用于安全控制的不确定性集合。逆感知契约（IPC）通过将感知估计映射到以高置信度包含真实值的集合来建立这种联系。然而，现有的 IPC 公式将不确定性实例化为单个椭球集，并依赖确定性信任分数来引导机器人运动。这种表示方式无法捕捉细粒度感知误差的多模态和不规则结构，往往导致集合过于保守并降低导航性能。在这项工作中，我们引入了基于高斯混合的逆感知契约（GM-IPC），它扩展了 IPC，利用源自高斯混合模型的椭球置信集并集来表示不确定性。这种设计超越了确定性的单集合抽象，能够在提供形式化保证的同时，捕捉细粒度、多模态和非凸的误差结构。本文提出了一个学习框架来训练 GM-IPC，使其兼顾概率包含、分布匹配和空空间惩罚，从而确保预测集合的有效性和紧凑性。我们进一步表明，所得的不确定性表征可用于下游规划框架以实现实时安全导航，在以概率方式保证安全的同时，实现更少保守且更具适应性的机器人运动。
