...
tags: ["query:hr-rob"]
score: 8.0
evidence: 基于Koopman神经动力学的全身安全控制
tldr: 针对具有强非线性、高维动力学的机器人系统在实时安全控制方面的挑战，本文提出了一种结合Koopman算子与安全集算法（SSA）的数据驱动框架。该方法通过Koopman算子将非线性动力学映射到高维线性空间，并利用SSA将跟踪与安全约束整合进单一的二次规划（QP）问题中求解。实验在Kinova Gen3机械臂和Go2四足机器人上验证了该方法在保证实时性的同时，实现了精确的轨迹跟踪与避障。
selection_source: fresh_fetch
motivation: 传统的非线性优化在处理具有复杂动力学和安全约束的机器人实时控制时计算开销巨大且难以保证可行性。
method: 提出一种学习Koopman嵌入与算子的数据驱动方法，并将其与安全集算法集成，在单一二次规划框架下解决控制问题。
result: 在机械臂和四足机器人上的实验表明，该方法能够实现高精度的运动跟踪并有效避开障碍物。
conclusion: 该研究证明了Koopman线性化模型与安全约束优化结合在提升复杂机器人系统实时安全控制性能方面的有效性。
---
