...
title_zh: FLARE-BO：面向低光照机器人视觉的基于贝叶斯优化的融合亮度与自适应 Retinex 增强
...
tldr: 针对低光照环境下机器人视觉感知受限的问题，本文提出了FLARE-BO框架。该方法无需训练，通过贝叶斯优化自动调节包括伽马校正、Retinex光照归一化、双边滤波及白平衡在内的8个关键参数。相比前代方法，它引入了更复杂的图像增强流水线和更高效的搜索策略，在LOL数据集上显著提升了图像质量，为自主机器人在恶劣光照下的导航与作业提供了更可靠的视觉支持。
source: arxiv
...
motivation: 现有的无需训练的低光照增强方法参数空间有限且去噪效果不佳，难以满足复杂机器人视觉任务的需求。
method: 提出FLARE-BO框架，利用贝叶斯优化在八维参数空间内联合优化光照分解、多种去噪算法及自动白平衡。
result: 在LOL数据集上的实验结果表明，该方法在图像增强质量上显著优于现有的非特定训练模型。
conclusion: FLARE-BO通过扩展参数空间和优化搜索策略，有效提升了低光照下机器人视觉感知的可靠性。
---

## 摘要
低光照下的可靠视觉感知仍然是自主机器人系统的核心挑战，退化的图像质量直接损害了导航、巡检和各种操作。最近的一种无需训练的方法表明，利用高斯过程的贝叶斯优化可以针对每张图像自适应地选择亮度、对比度和去噪参数，在没有任何学习模型的情况下实现了具有竞争力的增强效果。然而，该框架仅限于三个参数，没有应用光照分解或白平衡校正，并且依赖于非局部均值（NLM）去噪，这在噪声环境下往往会过度平滑边缘。本文提出了 FLARE-BO（基于贝叶斯优化的融合亮度与自适应 Retinex 增强），这是一个扩展框架，共同优化了涵盖伽马校正、LIME 风格光照归一化、色度去噪、双边滤波、NLM 去噪、灰度世界自动白平衡和自适应后平滑的八个参数。搜索引擎采用了单位超立方体参数归一化、目标标准化、Sobol 准随机初始化以及对数期望改进（Log Expected Improvement）采集函数，以对扩展空间进行原则性探索。该方法的性能在低光照配对数据集（LOL）上进行了基准测试，结果显示，与未在该数据集上专门训练的现有方法相比，所提方法有显著改进。
