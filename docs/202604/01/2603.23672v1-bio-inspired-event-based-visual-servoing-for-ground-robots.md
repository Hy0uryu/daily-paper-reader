...
tldr: 本研究受生物主动感知启发，提出一种基于事件相机的地面机器人视觉伺服框架。利用动态视觉传感器（DVS）捕获的异步事件流，通过对结构化光强变化模式应用固定空间核，直接从事件通量中解析出机器人的速度和位置等运动状态。该方法无需传统状态估计，并引入仿生主动感知极限环控制器解决平衡点观测性缺失问题，在1/10比例自动驾驶车辆上验证了其极低延迟和高计算效率。
selection_source: fresh_fetch
motivation: 旨在模仿生物感知的自适应特性，利用事件相机的高效性解决地面机器人视觉伺服中的计算开销和延迟问题。
method: 通过在结构化模式上应用线性和二次空间核处理事件流，直接合成非线性状态反馈项，并结合主动感知极限环控制。
result: 实验证明该方法无需传统状态估计即可实现精确控制，展现出极低的延迟和极高的计算效率。
conclusion: 该研究证明了直接利用事件通量进行机器人闭环控制的可行性，为低功耗、高动态响应的机器人感知提供了新思路。
---

## 摘要
生物感觉系统具有天生的适应性，能够过滤掉恒定刺激并优先处理相对变化，这可能提高了计算和代谢效率。受多种动物主动感知行为的启发，本文提出了一种新型的地面机器人基于事件的视觉伺服框架。利用动态视觉传感器（DVS），我们证明了通过对结构化对数光强变化模式产生的异步事件流应用固定空间核，所得的净事件通量可以在解析上分离出特定的运动状态。我们为该事件速率估计器建立了一个广义理论界限，并表明线性与二次空间轮廓分别分离了机器人的速度以及位置-速度乘积。利用这些特性，我们采用多模式刺激来直接合成非线性状态反馈项，完全无需传统的状态估计。为了克服事件感知中固有的平衡点处线性可观测性丧失这一不可避免的问题，我们提出了一种仿生主动感知极限环控制器。在 1/10 比例自主地面车辆上的实验验证确认了所提直接感知方法的有效性、极低延迟和计算效率。
