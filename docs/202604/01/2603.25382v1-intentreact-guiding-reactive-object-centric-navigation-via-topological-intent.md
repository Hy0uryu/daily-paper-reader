...
tldr: IntentReact 针对物体目标导航中全局拓扑规划与局部反应式控制脱节的问题，提出了一种意图引导的框架。该方法将全局拓扑路径编码为低维方向信号（意图），并以此调节局部航点预测策略。这使得机器人即使在局部观测受限时，也能根据全局拓扑一致性快速调整航向，在保持反应式控制鲁棒性的同时，显著提升了导航效率和成功率。
selection_source: fresh_fetch
motivation: 现有方法在局部决策时仅依赖当前视野，导致机器人在局部观测误导下难以做出符合全局拓扑最优路径的移动决策。
method: 引入一种紧凑的意图接口，将全局拓扑引导编码为方向信号，用以调节航点预测策略，使其偏向于减少全局拓扑距离的方向。
result: 在广泛的实验评估中，IntentReact 在导航成功率和执行质量方面均表现出优于此前物体中心导航方法的性能。
conclusion: 通过在意图层面耦合全局规划与局部控制，可以有效克服局部感知局限，实现更高效、更具鲁棒性的物体目标导航。
---

## 摘要
目标物体视觉导航要求机器人在部分可观测性下对语义结构进行推理并有效行动。近期基于对象级拓扑地图的方法实现了无需密集几何重建的长程导航，但其执行仍受限于全局拓扑引导与局部感知驱动控制之间的差距。具体而言，局部决策仅根据当前的自我中心观测做出，无法获取机器人视野之外的信息。结果是，即使初始朝向偏离目标，机器人也可能坚持当前的航向，向无法减小全局拓扑距离的方向移动。在这项工作中，我们提出了 IntentReact，这是一个以意图为条件的以对象为中心的导航框架，它在全局拓扑规划和反应式以对象为中心的控制之间引入了一个紧凑的接口。我们的方法将全局拓扑引导编码为一种低维方向信号，称为“意图”（intent），该信号作为条件约束学习到的路标点预测策略，使导航偏向于拓扑一致的进展。这种设计使机器人能够在局部观测产生误导时迅速重新调整方向，引导运动朝向减小全局拓扑距离的方向，同时保留了以对象为中心的控制的反应性和鲁棒性。我们通过广泛的实验评估了所提出的框架，证明了与之前的以对象为中心的导航方法相比，导航成功率和执行质量均有所提高。
