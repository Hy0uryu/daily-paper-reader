...
tldr: "针对灵巧手抓取中密集姿态规划难和端到端强化学习缺乏可控性的问题，本文提出了GRIT框架。该框架采用两阶段方法：首先根据场景和任务预测稀疏的抓取分类规范，随后以此为条件引导策略生成连续的手指运动。GRIT通过建立抓取分类与物体几何之间的联系，显著提升了对新物体的泛化能力，在仿真中达到87.9%的成功率，并在实物实验中展示了通过高层分类选择实现的可控性。"
source: arxiv
...
motivation: 传统的密集姿态规划难以大规模应用，而纯强化学习方法在复杂任务中缺乏可控性和可干预性。
method: 提出一种两阶段框架，先预测基于分类法的稀疏抓取指令，再训练条件策略执行符合该分类结构的灵巧操作。
result: "该方法在处理新物体时表现优于基线，实现了87.9%的总成功率，并支持通过高层指令灵活调整抓取策略。"
conclusion: 结合稀疏的抓取分类引导与底层运动控制，是实现高效、可控且具有泛化能力的灵巧手抓取的有效途径。
---

## 摘要
灵巧操纵需要规划适合物体和任务的抓取构型，随后通过协调的多指控制来执行。然而，为每个物体和任务指定具有密集姿态或接触目标的抓取计划是不切实际的。同时，仅依靠任务奖励的端到端强化学习缺乏可控性，使得用户在发生故障时难以干预。为此，我们提出了 GRIT，这是一个从稀疏分类法引导中学习灵巧控制的两阶段框架。GRIT 首先根据场景和任务上下文预测基于分类法的抓取规范。在该稀疏指令的约束下，策略生成连续的手指运动以完成任务，同时保持预期的抓取结构。我们的结果表明，某些抓取分类法对于特定的物体几何形状更为有效。通过利用这种关系，GRIT 相比基准模型提高了对新物体的泛化能力，并实现了 87.9% 的总成功率。此外，真实世界实验证明了其可控性，能够根据物体几何形状和任务意图，通过高层分类法选择来调整抓取策略。
