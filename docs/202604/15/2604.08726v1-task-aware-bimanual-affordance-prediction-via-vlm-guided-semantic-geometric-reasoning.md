...
tags: ["query:hr-rob"]
score: 8.0
evidence: 用于协同操作的双臂示能性预测
tldr: 针对双臂协作机器人中接触点定位与手臂分配难以兼顾语义意图的问题，本文提出一种基于视觉语言模型（VLM）引导的语义-几何推理框架。该方法将多视图RGB-D数据融合为3D场景，利用VLM的跨类别泛化能力，通过查询任务相关的可供性区域和手臂分配策略来过滤6-DoF抓取候选。在九类真实世界任务中，该方法显著提升了任务导向抓取的成功率，实现了非结构化环境下可靠的双臂操作。
source: arxiv
...
motivation: 现有的双臂操作方法缺乏对任务意图的语义理解，难以同时解决“在哪里交互”和“由哪只手操作”的联合推理问题。
method: 提出一种层次化框架，通过融合多视图3D表征并结合VLM的语义推理，对生成的6-DoF抓取候选进行空间与语义的双重过滤及手臂分配。
result: 在包括工具使用和人机移交在内的九种真实场景任务中，该方法在任务成功率上均优于仅依赖几何或简单语义的基准模型。
conclusion: 显式的语义推理与手臂分配策略是实现非结构化环境下复杂双臂协作任务的关键。
---
