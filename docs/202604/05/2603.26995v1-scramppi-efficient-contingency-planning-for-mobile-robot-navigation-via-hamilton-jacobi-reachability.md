...
## 摘要
自主机器人通常旨在完成常规行为并最小化代价，这使得它们在面对故障或非计划场景时较为脆弱。在这些场景下，需要一个通往安全集的备用或应急计划以避免任务彻底失败。我们将此问题形式化为在常规代价下带有安全约束的轨迹优化问题：即在常规计划的任何位置，都必须存在一条通往指定安全集的可行轨迹。以往的方法要么放宽这一硬约束，要么使用计算开销巨大的采样策略来优化该约束。与之不同，我们将此要求形式化为一个到达-规避（reach-avoid）问题，并利用 Hamilton-Jacobi (HJ) 可达性分析来验证应急计划的可行性。通过在环境信息揭示过程中在线计算安全集后向可达集的价值函数，并利用基于重采样的展开（rollouts）将其与基于采样的规划器（MPPI）集成，我们在保证满足硬约束的同时，显著提升了采样效率。最后，我们通过仿真和硬件实验展示了该算法在对抗性规避任务中，能够在移动机器人上实时生成常规和应急计划。
