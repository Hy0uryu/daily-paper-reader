...
tags: ["query:hr-rob"]
score: 6.0
evidence: 移动机器人的环境感知与目标搜索
tldr: 针对移动机器人在室内环境进行目标搜索时面临的观测不全和感知不确定性挑战，本文提出一种结合贝叶斯推理与深度强化学习（DRL）的混合框架。该方法通过贝叶斯推理在线更新目标位置的概率信念图，并利用DRL策略基于该表示进行导航决策。在Habitat 3.0仿真实验中，该方法显著提升了搜索成功率并降低了搜索成本，实现了高效且可靠的目标导航。
source: arxiv
...
motivation: 传统概率方法依赖手工启发式规则，而深度强化学习在处理部分可观测性时收敛慢且解释性差。
method: 提出一种混合框架，利用贝叶斯推理维护空间信念图，并训练强化学习策略直接从该概率表示中选择导航动作。
result: 在Habitat 3.0真实室内仿真环境中的评估表明，该方法在提高搜索成功率的同时减少了搜索路径长度。
conclusion: 结合贝叶斯信念估计与学习型动作选择，能有效提升移动机器人在不确定环境下的目标搜索效率和可靠性。
---

## 摘要
由于部分可观测性、感知不确定性以及探索与导航效率之间的权衡，室内环境下的移动机器人自主目标搜索具有挑战性。经典的概率方法显式地表示不确定性，但通常依赖于手工设计的动作选择启发式方法；而深度强化学习虽然能实现自适应策略，但往往面临收敛缓慢和可解释性有限的问题。本文提出了一种结合贝叶斯推理与深度强化学习的混合目标搜索框架。该方法维护一个关于目标位置的空间信念图，通过对校准后的目标检测结果进行在线贝叶斯推理来更新，并训练强化学习策略直接从该概率表示中选择导航动作。该方法在 Habitat 3.0 的逼真室内仿真环境中进行了评估，并与现有的基准策略进行了对比。在两个室内环境中的实验结果表明，所提方法在提高成功率的同时减少了搜索开销。总体而言，研究结果证明了将贝叶斯信念估计与学习型动作选择相结合的价值，能够在部分可观测条件下实现更高效、更可靠的目标搜索行为。
