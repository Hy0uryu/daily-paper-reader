...
tags: ["query:hr-rob"]
score: 7.0
evidence: 用于机器人操作和感知的触觉传感
tldr: 针对现有触觉语言模型缺乏力、几何形状等定量描述的问题，本文提出FG-CLTP预训练框架。通过构建包含10万余对触觉3D点云与语言描述的大规模数据集，并引入离散化数值标记机制实现定量语义对齐，显著提升了触觉感知的精细度。结合流匹配策略构建的3D-TLA架构在复杂操作任务中表现优异，为触觉-语言-动作模型奠定了坚实基础。
selection_source: fresh_fetch
motivation: 现有的触觉表示主要依赖定性描述，忽视了对精细操作至关重要的力大小、接触几何和方向等定量接触状态。
method: 提出一种细粒度对比语言触觉预训练框架，利用离散化数值标记机制将显式物理指标注入多模态特征空间。
result: "该模型在分类准确率上达到95.9%，回归误差降低了52.6%，且在跨模态迁移中表现出极小的虚实迁移差距。"
conclusion: FG-CLTP通过建立传感器无关的3D点云表示和细粒度对齐，为接触密集型机器人操作提供了鲁棒且通用的基础。
---
