...
motivation: 在非结构化环境中，机器人需要根据自身物理能力评估通行风险，而现有的视觉语言模型直接推理成本高且缺乏高效的缓存机制。
method: 提出一种结合视觉语义缓存和VLM轨迹选择的导航框架，实现具身感知的零样本代价地图生成与安全路径规划。
result: "实验表明，CATNAV在减少85.7%查询量的同时，将目标到达率提升了10个百分点，并减少了33%的行为约束违规。"
conclusion: CATNAV证明了通过视觉语义缓存和多模态推理，可以实现高效、安全且具备具身感知能力的零样本机器人导航。
---
