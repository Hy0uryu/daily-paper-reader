...
title_zh: ForceVLA2：通过力感知释放接触密集型操作中的力-位混合控制潜力
...
## 摘要
接触密集型操作的具身智能主要依赖于位置控制，而对交互力的显式感知和调节仍未得到充分探索，这限制了现实任务中的稳定性、精度和鲁棒性。我们提出了 ForceVLA2，这是一个端到端的视觉-语言-动作（VLA）框架，为机器人配备了力-位混合控制和显式力感知能力。ForceVLA2 在 VLM 专家模型中引入了基于力的提示，以构建跨阶段的力感知任务概念，并在动作专家模型中采用跨尺度混合专家（MoE）机制，将这些概念与实时交互力自适应融合，从而实现闭环力-位混合调节。为了支持学习和评估，我们构建了 ForceVLA2-Dataset，包含 5 个接触密集型任务（包括擦拭、按压和组装）的 1,000 条轨迹，涵盖多视图图像、任务提示、本体感受状态和力信号。大量实验表明，ForceVLA2 显著提高了接触密集型操作的成功率和可靠性，在 5 个任务中分别比 pi0 和 pi0.5 高出 48.0% 和 35.0%，并缓解了机械臂过载和接触不稳定等常见故障模式，从而积极推动了 VLA 中具有力感知的交互式物理智能的发展。项目主页见 https://sites.google.com/view/force-vla2/home。
