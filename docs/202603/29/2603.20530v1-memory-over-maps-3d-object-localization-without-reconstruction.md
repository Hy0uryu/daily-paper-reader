...
## 摘要
目标定位是导航和操作等具身任务的前提。传统方法依赖于构建显式的3D场景表示（如点云、体素网格或场景图）来实现目标定位。虽然有效，但这些流程会产生大量的建图时间、存储开销和可扩展性限制。视觉语言模型的最新进展表明，可以直接在2D观测上进行丰富的语义推理，这提出了一个根本性问题：物体定位是否需要完整的3D场景重建？在这项工作中，我们重新审视了物体定位，并提出了一种无地图流程，该流程仅将带有位姿的RGB-D关键帧存储为轻量级视觉记忆，而无需构建场景的任何全局3D表示。在查询时，我们的方法检索候选视图，使用视觉语言模型对其进行重新排序，并通过深度反投影和多视图融合构建查询目标的稀疏、按需3D估计。与基于重建的流程相比，这种设计大幅降低了预处理成本，使场景索引的构建速度提高了两个数量级以上，同时显著减少了存储空间。我们在下游的物体目标导航任务中进一步验证了定位的目标。尽管不需要特定任务的训练，我们的方法在多个基准测试中均取得了强劲的性能，证明了基于图像场景记忆的直接推理可以有效替代以物体为中心的机器人导航中的密集3D重建。项目页面：https://ruizhou-cn.github.io/memory-over-maps/
