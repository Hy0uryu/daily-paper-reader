...
tldr: 针对杂乱环境中目标抓取难题，本文提出GAPG框架，通过点云数据实现几何感知的推-抓协同。该框架利用抓取评估模块分析夹具与物体的几何关系，并引导推送模块预测动作对未来抓取空间的影响。实验证明，该方法在仿真和现实场景中均表现出更高的安全性、效率和泛化能力，有效解决了堆叠物体的操作难题。
selection_source: fresh_fetch
motivation: 现有推-抓协同方法因忽视场景几何信息，导致在处理堆叠或遮挡物体时抓取稳定性差且推送效率低下。
method: 提出一种基于点云的几何感知框架，通过分析夹具闭合区域内的点云分布评估抓取可行性，并据此指导推送动作以创造抓取空间。
result: 在仿真和真实环境的多种复杂场景下，该方法显著提升了抓取成功率和操作效率，并展现出对未知物体的良好泛化性。
conclusion: 结合几何信息的推-抓协同策略能有效将不可抓取状态转化为可抓取状态，是实现复杂环境下可靠机器人操作的关键。
---
