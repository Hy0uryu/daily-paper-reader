...
tldr: ManiDreams是一个开源的机器人操作框架，旨在解决现实世界操作中的不确定性问题。它通过将不确定性的表示、传播和约束集成到规划循环中，利用直觉物理模型实现鲁棒操作。该框架支持感知、参数和结构等多种不确定性来源，并采用“采样-预测-约束”循环来优化动作。实验证明，在面对各种扰动时，ManiDreams比传统强化学习基准表现出更强的鲁棒性，且具有高度的模块化和灵活性。
selection_source: fresh_fetch
motivation: 传统的动力学模型往往只关注最小化预测误差，而忽略了现实世界操作中固有的不确定性对鲁棒性的影响。
method: 提出了一个模块化框架，通过组合式抽象实现分布状态表示、后端无关的动力学预测以及声明式约束规范，构建“采样-预测-约束”的规划闭环。
result: 在ManiSkill任务和真实世界部署中，该框架在存在各种扰动的情况下仍能保持稳健性能，显著优于性能退化的强化学习基准。
conclusion: ManiDreams通过在规划层显式处理不确定性，为实现灵活且鲁棒的机器人操作提供了一个通用的开源解决方案。
---

## 摘要
动力学模型，无论是模拟器还是学习到的世界模型，长期以来一直是机器人操控的核心，但大多数模型侧重于最小化预测误差，而非面对一个更根本的挑战：现实世界的操控本质上是不确定的。我们认为，不确定性下的鲁棒操控从根本上说是一个集成问题：不确定性必须在规划循环中进行表示、传播和约束，而不仅仅是在训练期间被抑制。我们提出并开源了 ManiDreams，这是一个基于直觉物理模型进行不确定性感知操控规划的模块化框架。它通过可组合的抽象实现了这种集成，包括分布状态表示、后端无关的动力学预测以及用于动作优化的声明式约束规范。该框架明确解决了三种不确定性来源：感知不确定性、参数不确定性和结构不确定性。它使用“采样-预测-约束”循环封装任何基础策略，根据分布结果评估候选动作，在无需重新训练的情况下增强了鲁棒性。在 ManiSkill 任务上的实验表明，在强化学习（RL）基准性能显著下降的各种扰动下，ManiDreams 仍能保持鲁棒的性能。在推、取、接以及现实世界部署中的可运行示例展示了其在不同策略、优化器、物理后端和执行器之间的灵活性。该框架已在 https://github.com/Rice-RobotPI-Lab/ManiDreams 公开发布。
