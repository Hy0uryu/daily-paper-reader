...
title_zh: 粒子动力学快速适应在通用变形物体移动操作中的应用
...
score: 7.0
evidence: 柔性物体的移动操作
tldr: "本研究针对具有未知动力学特性的变形物体移动操作难题，提出了RAPiD方法。该方法借鉴了快速电机适应（RMA）的思想，通过在仿真中使用物体的粒子位置等特权信息编码动力学特征，并学习在现实中仅凭视觉和动作序列推断该特征。在22自由度移动机器人上的实验表明，RAPiD能有效处理多种变形物体的复杂操作任务，成功率超过80%，显著提升了机器人在复杂物理环境下的泛化能力。"
selection_source: fresh_fetch
motivation: 变形物体的动力学参数难以预知且对操作至关重要，而现有RMA方法主要针对刚体，无法直接处理复杂的形状变化。
method: 提出RAPiD两阶段框架，先在仿真中利用粒子位置等特权信息训练条件策略，再学习利用视觉观测和动作序列在线推断动力学嵌入以实现跨域迁移。
result: "在真实世界的移动机器人平台上，针对多种类别和动力学特性的变形物体，实现了超过80%的操作成功率。"
conclusion: 通过引入粒子动力学表征，成功将快速适应技术扩展到变形物体操作领域，证明了该方法在处理复杂非刚体任务中的有效性。
---

## 摘要
我们解决了学习操作具有未知动力学的变形物体的挑战。在非刚性物体中，动力学参数定义了它们对交互的反应方式——如拉伸、弯曲、压缩和移动——这些参数对于确定成功执行操作任务的最优动作至关重要。在其他机器人领域，如足式运动和手内刚性物体操作，最先进的方法可以使用快速电机适应（Rapid Motor Adaptation, RMA）来处理未知动力学。通过在仿真中对每个刚性物体的动力学（如质量和位置）进行编码的监督过程，这些方法学习了一种策略，该策略根据从状态-动作序列中推断出的潜在动力学参数矢量来调整动作。然而，在变形物体操作中，物体的动力学不仅包括其质量和位置，还包括物体形状的变化方式。我们的核心见解是，仿真中变形物体的近期真实粒子位置捕捉到了物体形状的变化，这使得将 RMA 扩展到变形物体操作成为可能。基于这一见解，我们开发了 RAPiD，这是一种分为两个阶段的方法，通过以下方式学习执行真实机器人的变形物体移动操作：1) 学习一种以物体动力学嵌入为条件的视觉运动策略，该嵌入由仿真中的物体特权信息（如质量和真实粒子位置）编码而成；2) 学习改用非特权信息（如机器人的视觉观测和动作）来推断该嵌入，从而使学习到的策略能够迁移到现实世界。在一个具有 22 个自由度的移动操作机上，RAPiD 在现实世界中两种基于视觉的变形物体移动操作任务中实现了超过 80% 的成功率，涵盖了各种物体动力学、类别和实例。
