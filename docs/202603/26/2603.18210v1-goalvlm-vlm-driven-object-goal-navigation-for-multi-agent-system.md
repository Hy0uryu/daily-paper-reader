...
## 摘要
物体目标导航传统上局限于具有闭集物体词汇表的地面机器人。现有的多智能体方法依赖于与固定类别集绑定的预计算概率图，这阻碍了在测试时泛化到新目标。我们提出了 GoalVLM，这是一个用于零样本、开放词汇物体导航的协作式多智能体框架。GoalVLM 将视觉语言模型 (VLM) 直接集成到决策循环中，利用 SAM3 进行文本提示的检测与分割，并使用 SpaceOM 进行空间推理，使智能体能够解释自由文本语言目标，并无需重新训练即可通过零样本语义先验对边界 (frontiers) 进行评分。每个智能体通过深度投影体素泼溅 (voxel splatting) 构建鸟瞰图 (BEV) 语义地图，同时目标投影器 (Goal Projector) 通过校准后的深度将检测结果反向投影到地图中，以实现可靠的目标定位。约束引导的推理层通过结构化提示链（场景描述、房间类型分类、感知门控、多边界排序）评估边界，将常识先验注入探索过程。我们在 GOAT-Bench val_unseen（包含 360 个多子任务回合、1032 个连续物体目标子任务、HM3D 场景）上评估了 GoalVLM，其中每个回合需要导航到由 5-7 个开放词汇目标组成的链条。具有 N=2 个智能体的 GoalVLM 实现了 55.8% 的子任务成功率 (SR) 和 18.3% 的成功路径长度加权 (SPL)，在无需特定任务训练的情况下，达到了与最先进方法相当的水平。消融实验证实了 VLM 引导的边界推理和深度投影目标定位的贡献。
