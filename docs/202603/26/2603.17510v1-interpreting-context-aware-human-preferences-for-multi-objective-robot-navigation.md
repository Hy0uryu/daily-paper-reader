...
title_zh: 面向多目标机器人导航的上下文感知人类偏好解释
...
## 摘要
在人类共享环境中运行的机器人不仅必须实现安全和效率等任务级导航目标，还必须使其行为适应人类偏好。然而，由于人类偏好通常以自然语言表达并依赖于环境上下文，因此很难将其直接集成到低级机器人控制策略中。在这项工作中，我们提出了一种流水线，通过将基础模型与多目标强化学习（MORL）导航策略相结合，使机器人能够理解并应用依赖于上下文的导航偏好。因此，我们的方法将高级语义推理与低级运动控制相结合。视觉语言模型（VLM）从机载视觉观测中提取结构化的环境上下文，而大语言模型（LLM）将自然语言用户反馈转换为可解释的、依赖于上下文的行为规则，并存储在持久但可更新的规则存储器中。随后，偏好转换模块将上下文信息和存储的规则映射为数值偏好向量，这些向量对预训练的 MORL 策略进行参数化，以实现实时导航自适应。我们通过定量的组件级评估、用户研究以及在各种室内环境中的真实机器人部署来评估所提出的框架。我们的结果表明，该系统能够可靠地捕捉用户意图，生成一致的偏好向量，并能在不同上下文中实现可控的行为自适应。总的来说，所提出的流水线提高了机器人在共享人类环境中运行的自适应性、透明度和可用性，同时保持了安全且响应迅速的实时控制。
