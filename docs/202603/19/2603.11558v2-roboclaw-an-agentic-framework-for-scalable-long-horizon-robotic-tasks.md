...
## 摘要
视觉-语言-动作（VLA）系统在语言驱动的机器人操纵方面展现出巨大潜力。然而，将其扩展到长程任务仍然具有挑战性。现有的流程通常将数据采集、策略学习和部署分开，导致对人工环境重置的高度依赖以及多策略执行的脆弱性。我们提出了 RoboClaw，这是一个智能体机器人框架，它在单一的视觉语言模型（VLM）驱动控制器下统一了数据采集、策略学习和任务执行。在策略层面，RoboClaw 引入了纠缠动作对（EAP），将前向操纵行为与逆向恢复动作耦合，形成用于自主数据采集的自重置循环。该机制能够以极少的人工干预实现持续的同策略数据获取和迭代策略优化。在部署期间，同一个智能体执行高层推理，并动态编排已学习的策略原语以完成长程任务。通过在采集和执行过程中保持一致的上下文语义，RoboClaw 减少了两个阶段之间的不匹配，并提高了多策略的鲁棒性。在真实世界操纵任务中的实验表明，与传统的开环流程相比，该框架提高了稳定性和可扩展性，同时显著减少了整个机器人生命周期中的人力投入，在长程任务上的成功率比基准方法提高了 25%，并减少了 53.7% 的人力时间投入。
