...
tldr: 针对机器人模仿学习中扩散模型推理效率低、计算开销固定的问题，本文提出GeCO框架。该方法将动作合成从轨迹积分转变为迭代优化，通过学习时间无关的平稳速度场，使专家行为成为稳定吸引子。GeCO实现了根据任务复杂度自适应分配计算资源，并利用速度场范数作为无需训练的分布外（OOD）检测信号，在提升推理效率的同时增强了部署安全性。
selection_source: fresh_fetch
motivation: 现有的扩散模型和流匹配策略在推理时采用固定的计算步数，无法根据任务复杂度自适应调整，且缺乏有效的安全检测机制。
method: 提出GeCO框架，通过学习动作序列空间中的平稳速度场，将动作生成转化为寻找稳定吸引子的迭代优化过程。
result: 在仿真基准测试和大规模VLA模型上的实验表明，GeCO能自适应分配计算量，并利用速度场范数实现鲁棒的分布外检测。
conclusion: GeCO作为一种即插即用的流匹配替代方案，在提高机器人控制成功率和效率的同时，为安全部署提供了原生的优化与检测机制。
---
