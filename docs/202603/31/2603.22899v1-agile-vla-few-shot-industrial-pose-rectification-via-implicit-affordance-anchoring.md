...
method: 引入隐式可操作性锚定机制，通过异步双流架构将 10Hz 的感知与 50Hz 的控制进行解耦。
result: 在 Jetson Orin Nano 上，仅需 5 次演示即可利用外部灵巧性完成复杂不规则工件的姿态纠正。
conclusion: 该研究证明了结合几何锚定与异步架构可使 VLA 模型在资源受限的边缘端实现高效的少样本工业操作。
---

## 摘要
在资源受限的边缘平台上部署视觉-语言-动作（VLA）模型时，高延迟的语义推理与动态操作所需的高频控制之间存在根本冲突。为了应对这一挑战，本文提出了 Agile-VLA，这是一个专为 NVIDIA Jetson Orin Nano 等边缘设备上的工业位姿重定向任务设计的分层框架。其核心创新在于一种隐式可操作性锚定机制，该机制将几何视觉线索（具体为质心和边缘关键点锚点）直接映射为结构化的参数化动作原语，从而大幅减少了闭环控制过程中对高延迟语义推理的依赖。通过异步双流架构将感知（10 Hz）与控制（50 Hz）解耦，该系统有效缓解了边缘机器人学习中固有的频率不匹配问题。在标准 6 自由度机械臂上的实验结果表明，Agile-VLA 仅需 5 次演示，即可通过外部灵巧性实现对复杂、不规则工件的稳健纠偏。
