...
tldr: 针对机器人动态导航中控制屏障函数（CBF）参数固定导致的保守或冒险问题，本文提出AlphaAdj框架。该框架利用视觉语言模型（VLM）根据第一视角RGB图像实时评估环境风险，并动态调节CBF参数以平衡安全与效率。通过引入速度感知动态上限和陈旧门控融合策略，解决了VLM推理延迟问题，在保证零碰撞的同时显著提升了导航效率和鲁棒性。
selection_source: fresh_fetch
motivation: 传统的CBF安全滤波器因参数固定，难以在复杂动态环境中兼顾避障的安全性与任务的执行效率。
method: 利用VLM从视觉输入中提取风险标量，动态调整CBF参数，并结合几何速度补偿和异步融合机制应对推理延迟。
result: "实验表明，AlphaAdj在多种场景下比固定参数方法提升了高达18.5%的路径效率，并显著增强了导航成功率。"
conclusion: 该研究证明了将大模型视觉感知与经典控制理论相结合，能有效提升机器人在非结构化环境中的实时安全导航能力。
---
