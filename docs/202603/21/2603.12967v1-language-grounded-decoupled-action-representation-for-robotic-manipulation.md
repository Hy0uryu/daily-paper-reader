...
tldr: 针对机器人操作中高层语义理解与底层动作控制之间的异构性挑战，本文提出LaDA框架。该框架利用自然语言作为语义桥梁，将动作解耦为平移、旋转和夹爪控制三个可解释原语。通过语义引导的软标签对比学习和自适应加权策略，LaDA增强了动作的泛化性与一致性。实验证明其在模拟和真实场景中均表现优异，能有效处理未见或相关任务。
