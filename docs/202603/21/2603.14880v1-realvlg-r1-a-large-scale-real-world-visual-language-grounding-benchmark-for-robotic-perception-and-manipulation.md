...
## 摘要
视觉语言定位（Visual-language grounding, VLG）旨在建立自然语言与视觉实体之间的语义对应关系，使模型能够根据文本指令准确识别和定位目标物体。现有的 VLG 方法侧重于粗粒度的物体级定位，而传统的机器人抓取方法主要依赖几何线索且缺乏语言引导，这限制了它们在语言驱动的操作场景中的适用性。为了解决这些局限性，我们提出了 RealVLG 框架，该框架集成了 RealVLG-11B 数据集和 RealVLG-R1 模型，以统一真实世界的视觉语言定位和抓取任务。RealVLG-11B 数据集提供了多粒度标注，包括边界框、分割掩码、抓取位姿、接触点以及经过人工验证的细粒度语言描述，涵盖了约 16.5 万张图像、800 多个物体实例、130 万个分割、检测和语言标注，以及约 110 亿个抓取示例。基于该数据集，RealVLG-R1 在预训练的大规模视觉语言模型上采用强化微调（Reinforcement Fine-tuning），以在给定自然语言指令的情况下，统一预测边界框、分割掩码、抓取位姿和接触点。实验结果表明，RealVLG 支持在真实世界的未知环境中进行零样本感知和操作，建立了一个统一的语义-视觉多模态基准，为语言驱动的机器人感知和抓取策略学习提供了一个全面的数据和评估平台。所有数据和代码均可在 https://github.com/lif314/RealVLG-R1 公开获取。
