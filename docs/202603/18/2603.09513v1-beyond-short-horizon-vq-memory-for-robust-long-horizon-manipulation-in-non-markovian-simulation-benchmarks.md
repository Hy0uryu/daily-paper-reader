...
tldr: 针对现有机器人仿真基准任务过于简单且缺乏非马尔可夫特性的问题，本文提出了RuleSafe基准，包含多种复杂解锁机制的长程任务。同时，提出VQ-Memory方法，通过VQ-VAE将历史本体感知状态编码为离散潜在令牌，为视觉-语言-动作（VLA）模型提供轻量且鲁棒的时间线索。实验证明该方法显著提升了长程规划能力、泛化性及操作效率。
selection_source: fresh_fetch
motivation: 现有机器人仿真基准多关注简单任务，难以模拟现实世界中具有非马尔可夫特性和复杂关节物体交互的长程任务。
method: 提出了RuleSafe基准和VQ-Memory表示法，后者利用向量量化变分自编码器将过去状态压缩为离散令牌以保留高层任务上下文。
result: 在SOTA VLA模型和扩散策略上的实验表明，VQ-Memory增强了长程规划能力，并提高了对未知配置的泛化效率。
